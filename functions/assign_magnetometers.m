...
%
%   meas_axes (optional) sets each sensor's axes, accepted as:
%     - omitted or []   : default axes depend on model dimension
%     - struct (1x1)    : fields axis1/axis2/axis3 (1x3 each) for 3D,
%                         or field axis (1x3) for 2D; broadcast to all sensors
%     - struct (1xN)    : N must equal num_sensors; one set per sensor
%
%   All axes are normalized to unit length (zero vectors error out).
%
%   Output: fmdl.sensors(m).position / .axes with normalized axes.
...
if ~ismember(dim, [2,3])
    error('fmdl.nodes must have size(fmdl.nodes,2) equal to 2 or 3');
end
...
if nargin < 3 || isempty(meas_axes)
    if dim == 3
        ...
        % Default: one out-of-plane measurement axis for every sensor
        meas_axes = repmat(struct('axis',[0,0,1]), 1, num_sensors);
    end
else
...
if ~isstruct(meas_axes)
    error('meas_axes must be a struct array with either axis1/axis2/axis3 or axis');
end

if isfield(meas_axes, 'axis1') || isfield(meas_axes, 'axis2') || isfield(meas_axes, 'axis3')
    validate_meas_axes_struct_3d(meas_axes);
...
elseif isfield(meas_axes, 'axis')
    validate_meas_axes_struct_2d(meas_axes);
    if numel(meas_axes) == 1
        meas_axes = repmat(meas_axes, 1, num_sensors);
    elseif numel(meas_axes) ~= num_sensors
        error('meas_axes must have length 1 or equal to num_sensors');
    ...
    for m = 1:numel(meas_axes)
        ...
        n = norm(v);
        if n == 0
            error('meas_axes(%d).axis is a zero vector and cannot be normalized', m);
        end
        meas_axes(m).axis = v / n;
    end
else
    error('meas_axes must have fields axis1/axis2/axis3 or axis');
end
end

function validate_meas_axes_struct_3d(meas_axes)
if ~isstruct(meas_axes)
...
function validate_meas_axes_struct_2d(meas_axes)
if ~isstruct(meas_axes)
    error('meas_axes must be a struct array (with field axis)');
end
if ~isfield(meas_axes, 'axis')
    error('meas_axes is missing required field: axis');
end
for m = 1:numel(meas_axes)
    v = meas_axes(m).axis;
    if numel(v) ~= 3 || ~isnumeric(v)
        error('meas_axes(%d).axis must be a 1x3 numeric vector', m);
    end
end
end